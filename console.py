...
class ALXCommand(cmd.Cmd):
    ...
    def do_all(self, args):
        ...
        elif line[0] in ALXCommand.class_list:
    ...
    def default(self, args):
        ...
        cmd_name = split2[0]
        if cmd_name == 'all':
            ALXCommand.do_all(self, class_name)
        elif cmd_name == 'count':
            count = 0
            ...
            if len(split2) < 2:
                ...
                ALXCommand.do_show(self, class_name + ' ' + split2[1])
        elif cmd_name == 'destroy':
            if len(split2) < 2:
                print('** no instance found **')
            else:
                ALXCommand.do_destroy(self, class_name + ' ' + split2[1])
            ...
            if len(split3) == 0:
                ...
                for k, v in split[1].items():
                    ALXCommand.do_update(self, class_name + ' ' + split3[0] +
                                          ' ' + k + ' ' + v)
            elif len(split3) == 1 and type(split3[1]) != dict:
                print('** no instance found **')
            elif len(split3) == 2:
    ...
    def verify_class(cls, line):
        ...
        if len(line) == 0:
            ...
            return False
        elif line[0] not in ALXCommand.class_list:
            print('** class doesn\'t exist **')
            return False
        return True
